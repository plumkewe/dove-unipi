// ============================================
    ...
    function loadOpenSeadragon() {
      ...
      osdLoadPromise = new Promise((resolve, reject) => {
          ...
          script.onerror = () =>
            reject(new Error("Failed to load OpenSeadragon"));
    ...
    function initApp() {
      ...
      const searchInputMobileElement = document.getElementById(
        "search-input-mobile",
      );
      ...
      function setupDelegatedListeners() {
        ...
        const searchResultsMobileContainer = document.getElementById(
        ...
        const handleSearchResultInteraction = (e) => {
          ...
          if (
            !resultItem ||
            resultItem.closest(".load-more-container") ||
            resultItem.querySelector('input[type="checkbox"]')
          )
            return;
      ...
      function handleSearchResultClick(e, element, result) {
        ...
        if (
          e.target.closest(".occupancy-status-bar") ||
          e.target.closest(".occupancy-expand-icon")
        ) {
          return;
        }
        ...
        if (result.suggestion) {
          const currentSearchInput = isMobile() ?
            searchInputMobile :
            searchInput;
          const otherSearchInput = isMobile() ?
            searchInput :
            searchInputMobile;
        ...
        if (result.type === "feedback") {
          if (result.action === "email") {
            window.open(
              "mailto:lyubomyr.malay@icloud.com?subject=" +
              encodeURIComponent("FEEDBACK DOVE?UNIPI"),
              "_blank",
            );
          } else if (result.action === "github") {
            window.open(
              "https://github.com/plumkewe/dove-unipi/issues/new",
              "_blank",
            );
          }
          const currentSearchInput = isMobile() ?
      ...
      function handleSearchResultMouseEnter(e, element, globalIndex) {
        ...
        if (
          isKeyboardSelection ||
          document.body.classList.contains("keyboard-navigation-active")
        ) {
          return;
        }
      ...
      function setupSidebarDelegation() {
        const sidebarContent =
          document.getElementById("rooms-list").parentElement; // Using parent to catch everything
        ...
        sidebarContent.addEventListener("click", (e) => {
          ...
          const scheduleBtn = e.target.closest(
            '[data-action="show-schedule"]',
          );
      ...
      function findRoomData(polo, building, floor, roomName) {
        if (!data.polo[polo] || !data.polo[polo].edificio[building])
          return null;
        ...
        setTimeout(() => {
          ...
          results.forEach((result) => {
            ...
            if (title && title.textContent === libName) {
              const expandIcon = result.querySelector(
                ".occupancy-expand-icon",
              );
              const detailsElement =
                result.querySelector(".occupancy-details");
      ...
      function handleRoomItemClick(
        ...
        const isClosing =
          roomInfoElement && !roomInfoElement.classList.contains("hidden");
        ...
        const mobileInput = typeof searchInputMobile !== "undefined" ? searchInputMobile : document.getElementById(
          "search-input-mobile");
        ...
        requestAnimationFrame(() => {
          ...
          if (roomInfoElement && roomInfoElement.innerHTML.trim() !== "") {
            ...
            // Only toggle the hidden class, do not clear the map marker or search
            roomInfoElement.classList.toggle("hidden");
            if (expandIcon) {
              expandIcon.style.transform = roomInfoElement.classList.contains(
                  "hidden",
                ) ?
                "rotate(0deg)" :
                "rotate(180deg)";
      ...
      const preventNativeZoomGestures = () => {
        ...
        const allowInsideViewer = (event) => {
          ...
          return (
            target &&
            (target.closest("#map") || target.closest("#viewer-container"))
          );
        };
      ...
      const showTooltip = (event) => {
        ...
        if (
          (target.id === "share-btn" || target.closest("#zoom-controls")) &&
      ...
      const miniMapToggleBtn = document.getElementById("polo-minimap-toggle-button");
      const poloMinimapPanel = document.getElementById("polo-minimap-panel");
      const poloMinimapContent = document.getElementById("polo-minimap-content");
      ...
      const clearSearchBtnMobile = document.getElementById(
        "clear-search-btn-mobile",
      );
      const settingsBtn = document.getElementById("settings-btn");
      const shareBtn = document.getElementById("share-btn");
      const accessibilityToggle = document.getElementById(
        "accessibility-toggle",
      );
      const storedShareCoordinates = localStorage.getItem(
        "shareCoordinatesEnabled",
      );
      ...
      const storedShowWaterDispensers = localStorage.getItem(
        "showWaterDispensersEnabled",
      );
      const storedShowStudyRooms = localStorage.getItem(
        "showStudyRoomsEnabled",
      );
      const storedShowClassroomStatus = localStorage.getItem(
        "showClassroomStatusEnabled",
      );
      const storedShowGroundFloor = localStorage.getItem(
        "showGroundFloorEnabled",
      );
      const storedCopyLinkOnSelect = localStorage.getItem(
        "copyLinkOnSelectEnabled",
      );
      ...
      const settingsConfig = {
        shareCoordinates: storedShareCoordinates ?
          storedShareCoordinates === "true" :
          true,
        highContrast: storedHighContrast ?
          storedHighContrast === "true" :
          false,
        dyslexicFont: storedDyslexicFont ?
          storedDyslexicFont === "true" :
          false,
        showWaterDispensers: storedShowWaterDispensers ?
          storedShowWaterDispensers === "true" :
          true,
        showStudyRooms: storedShowStudyRooms ?
          storedShowStudyRooms === "true" :
          true,
        showClassroomStatus: storedShowClassroomStatus ?
          storedShowClassroomStatus === "true" :
          true, // Default to true
        showGroundFloor: storedShowGroundFloor ?
          storedShowGroundFloor === "true" :
          true, // Default to true = "Piano Terra"
        copyLinkOnSelect: storedCopyLinkOnSelect ?
          storedCopyLinkOnSelect === "true" :
          false,
      ...
      async function loadTranslations(lang) {
        try {
          const response = await fetch(
            `${getSiteRootPath()}locales/${lang}.json`,
          );
      ...
      function applyTranslations() {
        ...
        if (metaOgDescription)
          metaOgDescription.content = t("meta_og_description");
        ...
        if (metaTwitterDescription)
          metaTwitterDescription.content = t("meta_og_description");

        // Update HTML lang attribute
        document.documentElement.lang = currentLanguage;

        // Update og:locale based on language
        const metaOgLocale = document.querySelector(
        ...
        if (searchInputMobile)
          searchInputMobile.placeholder = t("search_classroom");

        // Update tooltips (con controllo se esistono)
        if (openSidebarBtn)
          openSidebarBtn.setAttribute("data-tooltip", t("sidebar_open"));
        if (closeSidebarBtn)
          closeSidebarBtn.setAttribute("data-tooltip", t("sidebar_close"));
        if (shareBtn) {
        ...
        if (miniMapToggleBtn) {
          miniMapToggleBtn.setAttribute("data-tooltip", t("minimap_polo"));
          miniMapToggleBtn.setAttribute("aria-label", t("minimap_polo"));
        }
        if (zoomInBtn) {
      ...
      const savedAccessibilityState =
        localStorage.getItem("accessibilityEnabled") === "true";
      ...
      const markerRoomDataMap =
    new Map(); // Map per memorizzare i dati delle stanze associate ai marker (per event delegation)
      ...
      let currentBuildingSupportsFlip = false;
      let currentSelectionHasTopView = false;
      ...
      const SHORT_LINK_ELIGIBLE_TYPES = new Set([
        "aula",
        "dipartimento",
        "laboratorio",
        "sala",
      ...
      const STATIC_PAGE_EXCLUDED_TYPES = new Set([
        "persona",
        "erogatore_acqua",
        "laboratorio",
        "dipartimento",
      ...
      function getStaticPageTypes(room) {
        ...
        return new Set(
          [...roomTypes].filter(
            (roomType) => !STATIC_PAGE_EXCLUDED_TYPES.has(roomType),
          ),
        );
      }
      ...
      function buildCanonicalContextPath(
        poloName,
        buildingName = null,
        floorName = null,
      ) {
        ...
        if (floorName != null) {
          return buildCanonicalFloorPath(
            poloName,
            buildingName || "",
            floorName,
          );
        ...
        if (
          buildingName != null &&
          buildingName !== "" &&
          !hasSingleBuildingLayoutForPolo(poloName)
        ) {
          return buildCanonicalBuildingPath(poloName, buildingName);
        }
        return buildCanonicalPoloPath(poloName);
      ...
      function getRoomsOnFloor(poloName, buildingName, floorName) {
        const floorData =
          data?.polo?. [poloName]?.edificio?. [buildingName]?.piano?. [
            floorName
          ];
      ...
      function getCanonicalRoomSlug(
        ...
        for (const room of floorRooms) {
          ...
          if (
            room === targetRoom ||
            (room.id && targetRoom.id && room.id === targetRoom.id)
          ) {
            return slug;
          }
        }
      ...
      function buildCanonicalRoomPath(
        poloName,
        buildingName,
        floorName,
        room,
        ...
        const slug = getCanonicalRoomSlug(
          poloName,
          buildingName,
          floorName,
          room,
      ...
      function findRoomById(poloName, buildingName, floorName, roomId) {
        ...
        const room = getRoomsOnFloor(poloName, buildingName, floorName).find(
          (candidate) => candidate && candidate.id === roomId,
        );
      ...
      function findStaticDetailOnFloor(
        poloName,
        buildingName,
        floorName,
        detailId = null,
        ...
        for (const room of floorRooms) {
          ...
          if (
            (detailId && room.id && room.id === detailId) ||
            (detailSlug && candidateSlug === detailSlug)
          ) {
            return {
              polo: poloName,
              edificio: buildingName,
      ...
      function getRoomBaseShortCode(room) {
        ...
        if (Array.isArray(room.alias)) {
          const validAlias = room.alias.find(
      ...
      function generateRobustShortCode(poloName, targetRoom) {
        ...
        const index = conflictingRooms.findIndex(
          (item) => item.room === targetRoom,
        );
      ...
      function setCanonicalContextOverride({
        pageType,
        polo,
        building = null,
        floor = null,
      ...
      function getCanonicalContextOverridePath() {
        ...
        if (canonicalContextOverride.pageType === "polo")
          return canonicalContextOverride.path;
        if (canonicalContextOverride.building !== selectedBuilding)
          return null;
        if (canonicalContextOverride.pageType === "building")
          return canonicalContextOverride.path;
        if (
          canonicalContextOverride.pageType === "floor" &&
          canonicalContextOverride.floor === selectedFloor
        ) {
          return canonicalContextOverride.path;
        }
        return null;
      ...
      function getSearchResultsOpenMaxHeight(resultsElement) {
        return resultsElement === searchResultsMobile ?
          Math.floor(window.innerHeight * 0.5) :
          240;
      }

      function animateSearchResultsPanelResize(
        resultsElement,
        previousHeight = 0,
      ) {
      ...
      function animateSearchResultsPanelHide(
        resultsElement, {
          clearContent = false
        } = {},
      ) {
      ...
      const hideSearchResultsPanels = () => {
        ...
        const currentSearchResults = isMobile() ?
          searchResultsMobile :
          searchResults;
        const searchInputEl = isMobile() ? searchInputMobile : searchInput;
        const isPromoFn = (el) =>
          (el && el.innerHTML.includes('type: "bot_promo"')) ||
          (el && el.innerHTML.includes("bot_promo")); // Rough check due to innerHTML structure
        ...
        if (searchInputEl && searchInputEl.value.trim() === "") {
          const isPromoDismissed =
            localStorage.getItem("bot_v2_promo_dismissed") === "true";
          if (!isPromoDismissed) {
            // Don't hide
            return;
          }
      ...
      function updateClearButtonsVisibility() {
        ...
        if (clearSearchBtnMobile) {
          const hasValueMobile = Boolean(
            searchInputMobile &&
            searchInputMobile.value &&
            searchInputMobile.value.trim().length,
          );
      ...
      function updateSearchPlaceholder() {
        ...
        if (searchSuggestions.length <= 2) {
          ...
          do {
            newIndex =
              Math.floor(Math.random() * (searchSuggestions.length - 1)) + 1;
      ...
      function centerMapOnRoom(room) {
        if (
          !room ||
          !room.coordinates ||
          room.coordinates.x == null ||
          room.coordinates.y == null
        ) {
        ...
        const zoom =
          room.coordinates.zoom != null ?
          parseFloat(room.coordinates.zoom) :
          2;
      ...
      function clearSelectedRoomMarker() {
        ...
        if (mapContainer) {
          const hiddenPOIs = mapContainer.querySelectorAll(
            ".poi-hidden-for-selection",
          );
      ...
      function addSelectedRoomMarker(room, viewportPoint) {
        ...
        if (mapContainer) {
          ...
          const allMarkers = mapContainer.querySelectorAll(
            "[data-marker-type], .water-dispenser-marker, .study-room-marker",
          );
          allMarkers.forEach((marker) => {
            const markerRoomName = (
              marker.getAttribute("data-room-name") || ""
            ).toLowerCase();
      ...
      function matchesCapacity(capienza, filter) {
        const capacityValue =
          typeof capienza === "number" ? capienza : parseInt(capienza, 10);
      ...
      function searchRooms(query = "") {
        ...
        const matchesSettingsQuery =
          queryLower.includes("impostazioni") ||
          queryLower.includes("settings");
        const matchesShareQuery =
          queryLower.includes("share") ||
          queryLower.includes("condivid") ||
        ...
        const matchesFeedbackQuery = feedbackKeywords.some((kw) =>
          queryLower.includes(kw),
        );
        const matchesBotQuery =
          queryLower.includes("bot") || queryLower.includes("telegram");
        ...
        const specialKeywords = [{
            ...
            details: t("search_suggestion_settings_hint"),
          },
          {
            keywords: ["share", "condivid", "condivisione"],
            type: "share_suggestion",
            title: t("share_aria_label"),
            details: t("search_suggestion_share_hint"),
          },
          {
            keywords: [
              "feedback",
            ...
            title: "Feedback",
            details: t("search_suggestion_feedback_hint"),
          },
          {
            keywords: ["bot", "telegram"],
            type: "bot_suggestion",
            title: t("bot_title"),
            details: t("search_suggestion_bot_hint"),
          },
        ];
        ...
        if (shouldShowSuggestions) {
          for (const keyword of specialKeywords) {
            const partialMatch = keyword.keywords.some(
              (kw) => kw.startsWith(queryLower) && kw !== queryLower,
            );
          ...
          if (poloName.toLowerCase() === "ingegneria") {
            if (polo.edificio) {
              for (const bName in polo.edificio) {
                ...
                const defaultTitle =
                  b.alias && b.alias.length > 0 ?
                  b.alias[0] :
                  "Polo " + bName.toUpperCase();
                let matchedTitle = defaultTitle;
                let matches = false;

                const namesToSearch = (b.alternative_names || []).map(
                  (a) => ({
                    text: a,
                    display: a
                  }),
            ...
            const defaultTitle =
              "Polo " + poloName.charAt(0).toUpperCase() + poloName.slice(1);
            let matchedTitle = defaultTitle;
            let matches = false;

            const namesToSearch = (polo.alternative_names || []).map((a) => ({
              text: a,
              display: a,
            }));
        ...
        for (const poloName in data.polo) {
          ...
          for (const buildingName in polo.edificio) {
            ...
            if (
              building.dipartimenti &&
              Array.isArray(building.dipartimenti)
            ) {
              building.dipartimenti.forEach((dept) => {
                const ricercaLower = (dept.ricerca || "").toLowerCase();
                const aliasLower = Array.isArray(dept.alias) ?
                  dept.alias.map((a) => a.toLowerCase()) :
                  [];

                const matches = isExactSearch ?
                  ricercaLower === queryLower ||
                  aliasLower.some((a) => a === queryLower) :
                  ricercaLower.includes(queryLower) ||
                  aliasLower.some((a) => a.includes(queryLower));
              ...
              if (Array.isArray(items)) {
                items.forEach((item) => {
                  ...
                  if (!["persona", "erogatore_acqua"].includes(item.type)) {
                    ...
                    const aliasLower = Array.isArray(item.alias) ?
                      item.alias.map((a) => a.toLowerCase()) :
                      [];
                  ...
                  // 2. Check for Facilities (Erogatori)
                    ...
                    item.type === "erogatore_acqua" &&
                    settingsConfig.showWaterDispensers
                  ) {
                    const ricercaLower = (item.ricerca || "").toLowerCase();
                    const aliasLower = Array.isArray(item.alias) ?
                      item.alias.map((a) => a.toLowerCase()) :
        ...
        if (results.length > 0) {
          ...
          const getProximityScore = (item) => {
            ...
            if (
              item.polo === selectedPolo &&
              item.edificio === selectedBuilding &&
              item.piano === selectedFloor
            )
              return 0;
            if (
              item.polo === selectedPolo &&
        ...
        if (biblioteche.length > 0 && !capacityFilter) {
          ...
          for (const poloName in data.polo) {
            ...
            for (const buildingName in polo.edificio) {
              ...
              for (const floorName in building.piano || {}) {
                ...
                if (Array.isArray(items)) {
                  items.forEach((item) => {
                    const isLib = Array.isArray(item.type) ?
                      item.type.includes("biblioteca") :
                      item.type === "biblioteca";
          ...
          biblioteche.forEach((bib) => {
            ...
            const aliasLower = Array.isArray(bib.alias) ?
              bib.alias.map((a) => a.toLowerCase()) :
              [];
            const nomeLower = (bib.nome || "").toLowerCase();

            const matches = isExactSearch ?
              ricercaLower === queryLower ||
              nomeLower === queryLower ||
              aliasLower.some((a) => a === queryLower) :
              ricercaLower.includes(queryLower) ||
      ...
      function fetchLibrarySchedule(nid, container) {
        ...
        const getLiveContainer = () => {
          const liveStatusBar = document.querySelector(
            `.occupancy-status-bar[data-library-nid="${nid}"]`,
          );
          return liveStatusBar ?
            liveStatusBar.closest(".search-result-item") :
            container;
        };
        ...
        const directUrl =
          `https://www.sba.unipi.it/it/opening_hours/instances?from_date=${fromDate}&to_date=${toDate}&nid=${nid}`;
      ...
      function updateLibraryUI(container, data, isError = false) {
        ...
        if (!expandIcon) {
          ...
          expandIcon.className =
            "occupancy-expand-icon material-symbols-outlined";
      ...
      function displaySearchResults(
        ...
        const previousHeight = currentSearchResults.classList.contains(
            "hidden",
          ) ?
          0 :
        ...
        const finalizeAndAnimate = () => {
          animateSearchResultsPanelResize(
        ...
        if (!loadMore) {
          ...
          const oldNotification = currentSearchResults.querySelector(
            ".load-more-container",
          );
          if (oldNotification) {
        ...
        const endIndex = Math.min(
          startIndex + MAX_INITIAL_RESULTS,
          totalResults,
        );
        ...
        let otherPoliDividerInserted = loadMore ?
          currentSearchResults.querySelector(".search-results-divider") !==
          null :
          false;
        // Track if we've inserted the "Non su DOVE?UNIPI" divider
        let externalLibDividerInserted = loadMore ?
          currentSearchResults.querySelector(
            ".search-results-divider-external",
          ) !== null :
          false;
        // Track if we've inserted the polo_result section dividers
        let thisPoloDividerInserted = loadMore ?
          currentSearchResults.querySelector(
            ".search-results-divider-this-polo",
          ) !== null :
          false;
        let otherPoliPoloResultDividerInserted = loadMore ?
          currentSearchResults.querySelector(
            ".search-results-divider-other-polo",
          ) !== null :
          false;
        // Pre-check: are there polo_results for current AND other polos?
        const hasCurrentPoloResult = allSearchResults.some(
          (r) => r.type === "polo_result" && r.polo === selectedPolo,
        );
        const hasOtherPoloResult = allSearchResults.some(
          (r) => r.type === "polo_result" && r.polo !== selectedPolo,
        );
        ...
        resultsToShow.forEach((result, localIndex) => {
          ...
          if (
            !thisPoloDividerInserted &&
            result.type === "polo_result" &&
            result.polo === selectedPolo &&
            hasOtherPoloResult
          ) {
            const divider = document.createElement("div");
            divider.className =
              "search-results-divider search-results-divider-this-polo";
          ...
          if (
            !otherPoliPoloResultDividerInserted &&
            result.type === "polo_result" &&
            result.polo !== selectedPolo &&
            selectedPolo
          ) {
            const divider = document.createElement("div");
            divider.className =
              "search-results-divider search-results-divider-other-polo";
            divider.textContent =
              t("other_poli_polo_divider") || "Altri poli";
            fragment.appendChild(divider);
          ...
          if (
            ...
            divider.textContent =
              t("other_poli_divider") || "Risultati in altri poli";
            fragment.appendChild(divider);
          ...
          if (
            !externalLibDividerInserted &&
            result.type === "external_library"
          ) {
            const divider = document.createElement("div");
            divider.className =
              "search-results-divider search-results-divider-external";
            divider.textContent =
              t("not_on_dove_divider") || "Non su DOVE?UNIPI";
            fragment.appendChild(divider);
          ...
          if (result.type === "bot_promo") {
            resultElement.innerHTML = `
                        ...
                        `;
            const disableLink = resultElement.querySelector(
              ".disable-promo-link",
            );
            if (disableLink) {
          ...
          if (result.type === "share") {
            ...
            const capacityDetails =
              result.room && result.room.capienza != null ?
              result.room.capienza :
              null;
            const isFacility = result.isFacility || false;
            const isLibrary = Array.isArray(result.room.type) ?
              result.room.type.includes("biblioteca") :
              result.room.type === "biblioteca";

            // Aggiungi classe per gli erogatori d'acqua
            const isWaterDispenser = Array.isArray(result.room.type) ?
              result.room.type.includes("erogatore_acqua") :
              result.room.type === "erogatore_acqua";
            ...
            if (isLibrary) {
              ...
              settingsConfig.showClassroomStatus &&
              !isFacility &&
              shouldCheckStatus
            ) {
              const availabilitySource = window.classroomAvailability || {};
              const classroomName = result.room?.nome || "";
              const classroomKey = result.polo ?
                `${result.polo}:${classroomName}` :
                classroomName;
              const availabilityData =
                classroomKey && availabilitySource[classroomKey];
              ...
              if (availabilityData) {
                ...
                getClassroomAvailability(classroomKey)
                  .then((availability) => {
                    ...
                    const statusBars = currentSearchResults.querySelectorAll(
                      `.occupancy-status-bar[data-classroom="${CSS.escape(classroomKey)}"]`,
                    );
                    statusBars.forEach((statusBar) => {
                      statusBar.classList.remove("loading");
                      const statusTextSpan =
                        statusBar.querySelector(".occupancy-text");
                      const availabilityData =
                        window.classroomAvailability[classroomKey];
                      ...
                      if (availabilityData.code === 0) {
                        ...
                        if (statusTextSpan)
                          statusTextSpan.textContent = t("availability_free");
                      } else if (availabilityData.code === 1) {
                        statusBar.classList.add("occupied");
                        if (statusTextSpan)
                          statusTextSpan.textContent = t(
                            "availability_occupied",
                          );
                        ...
                        if (
                          ...
                          const occupancyId =
                            `occupancy-${classroomKey.replace(/[\s:]/g, "-")}-${Math.random().toString(36).substr(2, 9)}`; // Unique ID for each instance

                          // Create expand icon if not exists
                          if (
                            !statusBar.querySelector(".occupancy-expand-icon")
                          ) {
                            ...
                            statusBar.addEventListener("click", (e) => {
                              e.stopPropagation();
                              const detailsId =
                                expandIcon.getAttribute("data-occupancy-id");
                              const detailsElement =
                  ...
                  .catch((error) => {
                    console.warn(
                      `Failed to get availability for ${classroomName}:`,
                      error,
                    );
                    window.classroomAvailability[classroomKey] = {
                      code: 2,
              ...
              const occupancyId =
                `occupancy-${classroomKey.replace(/[\s:]/g, "-")}-${Math.random().toString(36).substr(2, 9)}`;
            ...
            const roomDetails =
              result.isPerson && result.room.room ?
            ...
            const buildingPathSegment =
              result.edificio !== "" ?
              `${t("building")} ${result.edificio.toUpperCase()}${(() => {
                      const bd =
                        data.polo[result.polo]?.edificio?.[result.edificio];
                      const al = bd?.alias;
                      return al && al.length > 0 ? " (" + al[0] + ")" : "";
                    })()} <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: -3px;">chevron_right</span>` :
              "";
            resultElement.innerHTML = `
            ...
            if (!isLibrary) {
              const statusBar = resultElement.querySelector(
                ".occupancy-status-bar",
              );
              const expandIcon = resultElement.querySelector(
                ".occupancy-expand-icon",
              );
        ...
        if (currentResultsShown < totalResults) {
          ...
          loadMoreContainer.className =
            "search-result-item load-more-container";
          ...
          const loadMoreLink =
            loadMoreContainer.querySelector(".load-more-link");
        ...
        const items = currentSearchResults.querySelectorAll(
          ".search-result-item",
        );
      ...
      function renderSettingsBlock(container) {
        ...
        settingsItems.forEach((config) => {
          ...
          row.className =
            "search-result-item flex items-center justify-between gap-4";
      ...
      function createSettingsToggle({
        ...
        slider.className =
          "w-11 h-6 bg-gray-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-gray-800";
      ...
      function createTextSizeControl({
        label,
        description
      }) {
        const wrapper = document.createElement("div");
        ...
        controlsContainer.className =
          "text-size-controls flex-shrink-0 self-end";
        ...
        function updateTextSize() {
          const currentSize = textSizes.findIndex((size) =>
            document.body.classList.contains(size),
          );
      ...
      function createLanguageSelector({
        ...
        const controlsContainer = document.createElement("div");
        controlsContainer.className =
          "language-selector flex-shrink-0 self-end";
      ...
      async function setLanguageSetting(lang) {
        ...
        if (success) {
          ...
          if (
            currentSearchResults &&
            !currentSearchResults.classList.contains("hidden") &&
            currentSearchInput
          ) {
            searchRooms(currentSearchInput.value);
          }
      ...
      function syncTextSizeControls() {
        ...
        document
          .querySelectorAll(".text-size-controls")
          .forEach((controls) => {
      ...
      function syncSettingsToggles() {
        document
          .querySelectorAll("input[data-settings-toggle]")
          .forEach((input) => {
      ...
      function updateSelectedSearchResult(index) {
        ...
        if (
          selectedSearchResultIndex > -1 &&
          items[selectedSearchResultIndex]
        ) {
      ...
      function setupSearchListeners(
        ...
        inputElement.addEventListener("input", (e) => {
          ...
          const otherInput =
            inputElement === searchInput ? searchInputMobile : searchInput;
        ...
        inputElement.addEventListener("keydown", (e) => {
          ...
          if (e.key === "ArrowDown") {
            ...
            document.addEventListener(
              "mousemove",
              handleMouseMoveDuringKeyboardNav,
            );
            e.preventDefault();
            ...
            e.preventDefault();
            const newIndex =
              (selectedSearchResultIndex - 1 + items.length) % items.length;
            updateSelectedSearchResult(newIndex);
            // Scroll into view only when using arrow keys
            if (items[newIndex]) {
              items[newIndex].scrollIntoView({
            ...
            if (
              ...
              const textSizeControls = currentItem.querySelector(
                ".text-size-controls",
              );
              const languageSelector =
                currentItem.querySelector(".language-selector");
            ...
            const clickResultItem = (itemElement) => {
              const isSpecialResult =
                itemElement.querySelector(
                  ".material-symbols-outlined[data-copy-icon]",
                ) ||
            ...
            if (primaryNameItems.length === 1) {
              ...
              const hasMultiButtons =
                itemElement.querySelector(".text-size-controls") ||
              ...
              if (firstItem) {
                const isSuggestion =
                  firstItem
                   ...
                   ?.textContent.trim() === "keyboard_return";
                if (isSuggestion) {
            ...
            if (
              clearButtonElement &&
              !clearButtonElement.classList.contains("hidden")
            ) {
      ...
      document.addEventListener("click", (e) => {
        ...
        if (
          ...
          e.target.classList.contains("disable-promo-link")
        ) {
          return;
        }
      ...
      if (settingsBtn) {
        settingsBtn.addEventListener("click", () => {
          const input =
            window.innerWidth <= 768 ? searchInputMobile : searchInput;
          if (input) {
      ...
      shareBtn.addEventListener("click", () => {
        ...
        navigator.clipboard
          ...
          .then(() => {
            ...
            showTooltip({
              currentTarget: shareBtn
            }); // Mostra subito il nuovo tooltip
            setTimeout(() => {
      ...
      function updateURL(replace = false) {
        ...
        const contextPath =
          getCanonicalContextOverridePath() ||
          buildCanonicalContextPath(
            selectedPolo,
            selectedBuilding,
            selectedFloor,
          );
        ...
        if (canUseShortLink) {
          if (
            shortLinkContext.canonicalRoomPage &&
            shortLinkContext.canonicalPath
          ) {
            ...
            const queryBasePath =
              shortLinkContext.queryBasePath || contextPath || appIndexPath;
        ...
        const resolvedHistoryUrl = new URL(newUrl, window.location.href);
        let historyUrl = `${resolvedHistoryUrl.pathname}${resolvedHistoryUrl.search}${resolvedHistoryUrl.hash}`;
        ...
        const roomName =
          shortLinkContext?.polo === selectedPolo ?
          (shortLinkContext?.roomName ?? null) :
          null;
        const buildingAlias = selectedBuilding ?
          data?.polo?. [selectedPolo]?.edificio?. [selectedBuilding]?.alias?. [0] :
          null;
        
        let titleDisplay;
        if (roomName) {
          titleDisplay = roomName;
        } else if (selectedBuilding && selectedPolo === 'fibonacci') {
          // For Fibonacci buildings, show "Edificio X"
          titleDisplay = `Edificio ${selectedBuilding.toUpperCase()}`;
        } else {
          // Default: use building alias or polo name
          titleDisplay = buildingAlias ||
            `Polo ${selectedPolo.charAt(0).toUpperCase() + selectedPolo.slice(1)}`;
        }
        
        document.title = `${titleDisplay} | DOVE?UNIPI`;
      }
      ...
      function handleInitialLoad() {
        ...
        let poloToSelect =
          poloParam && data.polo[poloParam] ? poloParam : polos[0];
        ...
        if (staticContextState?.polo && staticContextState?.pageType) {
          ...
          if (
            staticContextState.pageType === "detail" &&
            staticContextState.detail
          ) {
        ...
        if (
          ...
          staticRoomState?.building != null &&
          staticRoomState?.floor != null &&
          (staticRoomState?.roomId || staticRoomState?.slug)
        ) {
        ...
        if (roomFromShortLink) {
          ...
          // Use ricerca field for consistent display
          let displayName =
            roomFromShortLink.room.ricerca ||
            roomFromShortLink.matchedPersonName ||
            roomFromShortLink.room.nome;
          if (!displayName) {
            displayName =
              roomFromShortLink.room.id ||
              roomFromShortLink.room.slug ||
              roomParam;
          ...
          if (displayName) {
            ...
            if (searchInputMobile)
              searchInputMobile.value = initialSearchQuery;
          }
        }
      ...
      async function loadDataAndInitialize() {
        ...
        const networkFetchPromise = (async () => {
          const response = await fetch(
            `${getSiteRootPath()}data/unified.json?v=1.0.1`,
          );
          if (!response.ok) {
        ...
        fetch(`${getSiteRootPath()}data/biblioteche.json`)
          .then((r) => (r.ok ? r.json() : []))
          .then((json) => {
            biblioteche = Array.isArray(json) ? json : [];
          })
          .catch(() => {
            biblioteche = [];
          });
      ...
      function populatePolos(polos) {
        ...
        polos.forEach((polo) => {
          ...
          button.className =
            "nav-item flex flex-col items-start p-2 rounded-lg w-full text-left";
      ...
      function setMiniMapRotation(polo, shouldFlip) {
        ...
        if (!svg._rotationData) {
          ...
          const texts = Array.from(
              svg.querySelectorAll('text[transform*="rotate"]'),
            )
            .map((t) => {
              const tm = t
                .getAttribute("transform")
      ...
      function updateMiniMapState() {
        ...
        const supportsRotation =
          building && data.polo[selectedPolo]?.edificio[building]?.rotate;
      ...
      function renderPoloMiniMapPanel() {
        ...
        fetch(miniMapPath)
          .then((response) => {
            if (!response.ok) throw new Error("Mini-map not found");
            return response.text();
          })
          .then((svgText) => {
            poloMinimapContent.innerHTML = svgText;
            const svg = poloMinimapContent.querySelector("svg");
            if (!svg) return;

            svg.style.width = "100%";
            svg.style.height = "auto";
            svg.style.maxHeight = window.innerWidth <= 767 ? "150px" : "220px";
            svg.classList.add("mini-map-svg");
            svg.dataset.polo = selectedPolo;

            updateMiniMapState();
          })
          .catch(() => {
            poloMinimapContent.innerHTML =
              `<div style="font-size:.875rem;color:var(--color-gray-600);padding:.5rem;">
                 Mini-mappa non disponibile
               </div>`;
          });
      }

      function updatePoloMiniMapPanelPosition() {
        if (!poloMinimapPanel || !miniMapToggleBtn) return;

        const isMobileLayout = window.matchMedia("(max-width: 1200px)").matches;
        if (isMobileLayout) {
          const buttonRect = miniMapToggleBtn.getBoundingClientRect();
          const panelWidth = Math.min(260, window.innerWidth - 32);
          const minSidePad = 16;
          let panelLeft = buttonRect.right + window.scrollX - panelWidth;
          panelLeft = Math.max(minSidePad, Math.min(panelLeft, window.innerWidth - panelWidth - minSidePad));
          const panelTop = buttonRect.bottom + window.scrollY + 8;

          poloMinimapPanel.style.top = `${panelTop}px`;
          poloMinimapPanel.style.left = `${panelLeft}px`;
          poloMinimapPanel.style.right = "auto";
          poloMinimapPanel.style.width = `${panelWidth}px`;
          return;
        }

        // Desktop: anchored top-right below the button group
        const buttonRect = miniMapToggleBtn.getBoundingClientRect();
        poloMinimapPanel.style.top = `${buttonRect.bottom + 8}px`;
        poloMinimapPanel.style.right = `calc(1rem + env(safe-area-inset-right))`;
        poloMinimapPanel.style.left = "auto";
        poloMinimapPanel.style.width = "min(340px, calc(100vw - 2rem))";
      }

      function closeMinimapPanel() {
        if (poloMinimapPanel) poloMinimapPanel.classList.add("minimap-panel-hidden");
      }
      ...
      if (miniMapToggleBtn) {
        miniMapToggleBtn.addEventListener("click", (e) => {
          ...
          const willShow = poloMinimapPanel.classList.contains("minimap-panel-hidden");
          if (willShow) {
            openMinimapPanel();
          } else {
            closeMinimapPanel();
      ...
      document.addEventListener("click", (e) => {
        if (!poloMinimapPanel || poloMinimapPanel.classList.contains("minimap-panel-hidden")) return;
        if (poloMinimapPanel.contains(e.target)) return;
        if (miniMapToggleBtn && (e.target === miniMapToggleBtn || miniMapToggleBtn.contains(e.target))) return;
        closeMinimapPanel();
      });
      ...
      // Show/hide minimap toggle button based on polo having a miniMap
      function updateMiniMapButtonVisibility() {
        if (!miniMapToggleBtn) return;
        const poloConfig = data?.polo?.[selectedPolo];
        const hasMiniMap = poloConfig?.miniMap === true;
        miniMapToggleBtn.classList.toggle("hidden", !hasMiniMap);
        // Show/hide the parent container
        if (minimapControls) {
          minimapControls.classList.toggle("hidden", !hasMiniMap);
        }
        if (!hasMiniMap) {
          closeMinimapPanel();
      ...
      function selectPolo(
        ...
        if (buildings.length > 0) {
          const building =
            buildingToSelect && buildings.includes(buildingToSelect) ?
            buildingToSelect :
            buildings[0];
          selectBuilding(
            building,
      ...
      function populateBuildings(buildings) {
        ...
        buildings.forEach((building) => {
          const button = document.createElement("button");
          button.className =
            "nav-item w-full text-left flex items-center space-x-3 p-2 pl-6 rounded-lg";
      ...
      function populateFloors(building) {
        const floors = Object.keys(
          data.polo[selectedPolo].edificio[building].piano,
        ).sort((a, b) => a - b);
        floorsList.innerHTML = "";
      ...
      function populateRooms(building, floor) {
        ...
        const floorData =
          data.polo[selectedPolo].edificio[building]?.piano[floor];
        ...
        const floorDepartments = rooms.filter((room) =>
          roomHasType(room, "dipartimento"),
        );
        const departments = [...floorDepartments, ...buildingDepartments];
        const libraries = rooms.filter((room) =>
          roomHasType(room, "biblioteca"),
        );
        const studyRooms = rooms.filter((room) =>
          roomHasType(room, "studio"),
        );
        const laboratories = rooms.filter((room) =>
        ...
        const allRegularRooms = rooms.filter(
          (room) =>
          !roomHasType(room, "dipartimento") &&
          !roomHasType(room, "biblioteca") &&
        ...
        const sale = allRegularRooms.filter((room) =>
          roomHasType(room, "sala"),
        );
        const aule = allRegularRooms.filter(
          (room) => !roomHasType(room, "sala"),
        );
        ...
        const oldDeptContainer = document.getElementById(
          "temp-dept-container",
        );
        ...
        const oldStudyContainer = document.getElementById(
          "temp-study-container",
        );
        ...
        const oldSaleContainer = document.getElementById(
          "temp-sale-container",
        );
        ...
        const oldPeopleContainer = document.getElementById(
          "temp-people-container",
        );
        ...
        function createRoomItem(room, iconName = "door_front") {
          ...
          roomElement.className =
            "nav-item w-full text-left flex items-center justify-between space-x-3 p-2 pl-6 rounded-lg";
          ...
          roomInfoElement.className =
            "capacity-info hidden pl-10 p-1 text-sm text-gray-600";
          ...
          addInfoRow(
            room.accesso_disabili,
            "accessible_forward",
            t("room_detail_accessible"),
          );
          addInfoRow(
            room.accesso_disabili === false,
            "not_accessible_forward",
            t("room_detail_not_accessible"),
          );
          ...
          if (room.numero_prese_elettriche > 0) {
            addInfoRow(
              room.numero_prese_elettriche,
              "bolt",
              t("room_detail_power_outlets_count"),
            );
          } else {
            addInfoRow(
              room.prese_elettriche,
              "bolt",
              t("room_detail_power_outlets"),
            );
          }
          ...
          if (
            !addInfoRow(
              room.numero_pc,
              "computer",
              t("room_detail_computers_count"),
            )
          )
            addInfoRow(
              room.presenza_pc,
              "computer",
              t("room_detail_computers"),
            ); // In case we don't know the amount
          ...
          addInfoRow(
            room.altoparlanti,
            "speaker_group",
            t("room_detail_speakers"),
          );
          addInfoRow(
            room.telecamera,
            "speed_camera",
            t("room_detail_camera"),
          );
          ...
          const expandIconStyle = hasDetails ?
            "font-size: 20px; line-height: 1; transition: transform 0.2s;" :
            "display: none;";
        ...
        if (departments.length > 0) {
          ...
          departments.forEach((dept) => {
            ...
            deptElement.className =
              "department-item w-full text-left p-2 pl-6";
            ...
            if (dept["link-mappa"]) {
              ...
              mapBtn.className =
                "flex items-center space-x-2 p-2 bg-gray-300 rounded-lg hover:bg-[#b7bcc5] transition-colors";
              mapBtn.innerHTML =
                `<span class="material-symbols-outlined" style="font-size: 16px;">map</span><span>${t("map_button")}</span>`;
            ...
            if (dept["link-dipartimento"]) {
              ...
              deptBtn.className =
                "flex items-center space-x-2 p-2 bg-gray-300 rounded-lg hover:bg-[#b7bcc5] transition-colors";
              deptBtn.innerHTML =
                `<span class="material-symbols-outlined" style="font-size: 16px;">language</span><span>${t("website_button")}</span>`;
        ...
        if (libraries.length > 0) {
          ...
          libraries.forEach((lib) => {
            ...
            libHeader.className =
              "w-full text-left p-2 pl-6 rounded-lg cursor-pointer transition-colors flex items-center justify-between";
            ...
            if (lib.link_sito) {
              ...
              linkBtn.className =
                "flex items-center space-x-2 p-2 bg-gray-300 rounded-lg hover:bg-[#b7bcc5] transition-colors inline-flex text-gray-900";
              linkBtn.innerHTML =
                `<span class="material-symbols-outlined" style="font-size: 16px;">language</span><span>${t("website_button")}</span>`;
              ...
              hoursBtn.className =
                "flex items-center space-x-2 p-2 bg-gray-300 rounded-lg hover:bg-[#b7bcc5] transition-colors inline-flex text-gray-900";
              hoursBtn.innerHTML =
                `<span class="material-symbols-outlined pointer-events-none" style="font-size: 16px;">schedule</span><span class="pointer-events-none">${t("hours_button")}</span>`;
            ...
            roomInfoElement.className =
              "capacity-info hidden pl-10 p-1 text-sm text-gray-600 mt-2";

            addInfoRow(lib.capienza, "group", t("room_detail_capacity"));
            addInfoRow(
              lib.accesso_disabili,
              "accessible_forward",
              t("room_detail_accessible"),
            );
            addInfoRow(
              lib.accesso_disabili === false,
              "not_accessible_forward",
              t("room_detail_not_accessible"),
            );
            ...
            if (lib.numero_prese_elettriche > 0) {
              addInfoRow(
                lib.numero_prese_elettriche,
                "bolt",
                t("room_detail_power_outlets_count"),
              );
            } else {
              addInfoRow(
                lib.prese_elettriche,
                "bolt",
                t("room_detail_power_outlets"),
              );
            }
        ...
        if (sale.length > 0) {
          ...
          saleHeader.textContent =
            currentLanguage === "it" ? "Sale" : "Rooms"; // Using 'Rooms' for 'Sale' as translation, or maybe 'Halls'
          saleHeader.className =
            "text-sm font-semibold text-gray-500 uppercase tracking-wider mb-2";
        ...
        if (studyRooms.length > 0) {
          ...
          studyRooms.forEach((studyRoom) => {
            ...
            addInfoRow(
              studyRoom.capienza,
              "group",
              t("room_detail_capacity"),
            );
            addInfoRow(
              studyRoom.accesso_disabili,
              "accessible_forward",
              t("room_detail_accessible"),
            );
            addInfoRow(
              studyRoom.accesso_disabili === false,
              "not_accessible_forward",
              t("room_detail_not_accessible"),
            );
            addInfoRow(studyRoom.rete, "wifi", t("room_detail_network"));
            addInfoRow(
              studyRoom.porte_rete,
              "lan",
              t("room_detail_network_ports"),
            );
            if (studyRoom.numero_prese_elettriche > 0) {
              addInfoRow(
                studyRoom.numero_prese_elettriche,
                "bolt",
                t("room_detail_power_outlets_count"),
              );
            } else {
              addInfoRow(
                studyRoom.prese_elettriche,
                "bolt",
                t("room_detail_power_outlets"),
              );
            }
            if (
              ...
              addInfoRow(
                studyRoom.presenza_pc,
                "computer",
                t("room_detail_computers"),
              );
            addInfoRow(
              studyRoom.proiettore,
              "cast",
              t("room_detail_projector"),
            );
            addInfoRow(
              studyRoom.altoparlanti,
              "speaker_group",
              t("room_detail_speakers"),
            );
            addInfoRow(
              studyRoom.telecamera,
              "speed_camera",
              t("room_detail_camera"),
            );
            addInfoRow(
            ...
            studyRoomHeader.addEventListener("click", (e) => {
              ...
              const isClosing =
                roomInfoElement &&
                !roomInfoElement.classList.contains("hidden");
              ...
              requestAnimationFrame(() => {
                ...
                if (hasDetails) {
                  roomInfoElement.classList.toggle("hidden");
                  const expandIcon =
                    studyRoomHeader.querySelector(".expand-icon");
                  if (expandIcon) {
        ...
        const peopleList = (
          data?.polo?. [selectedPolo]?.edificio?. [selectedBuilding]?.piano?. [
            floor
          ] || []
        ...
        if (peopleList.length > 0) {
          ...
          allPeopleEntries.forEach((entry) => {
            ...
            personElement.className =
              "nav-item w-full text-left flex items-center justify-between space-x-3 p-2 pl-6 rounded-lg"; // Reduced padding for people
            ...
            if (pData.room) {
              addDetailRow(
                "meeting_room",
                `<span>${t("room_number").replace("%s", pData.room)}</span>`,
              );
            }
            ...
            if (pData.email) {
              addDetailRow(
                "mail",
                `<a href="mailto:${pData.email}" class="hover:underline truncate block" title="${pData.email}">${pData.email}</a>`,
              );
            }
            ...
            if (pData.tel) {
              addDetailRow(
                "call",
                `<a href="tel:${pData.tel.replace(/\s/g, "")}" class="hover:underline">${pData.tel}</a>`,
              );
            }
            ...
            if (pData.cv_link) {
              addDetailRow(
                "history_edu",
                `<a href="${pData.cv_link}" target="_blank" class="hover:underline">CV</a>`,
              );
            }
            ...
            if (pData.url) {
              addDetailRow(
                "language",
                `<a href="${pData.url}" target="_blank" class="hover:underline">Sito Web</a>`,
              );
            }
            ...
            if (pData.unimap_url) {
              addDetailRow(
                "link",
                `<a href="${pData.unimap_url}" target="_blank" class="hover:underline">UniMap</a>`,
              );
            }
            ...
            const firstName =
              pData.nome && typeof pData.nome === "string" ?
              pData.nome :
              entry.name.replace(surname, "").trim();
            ...
            personElement.addEventListener("click", (e) => {
              ...
              const isClosing =
                personInfoElement &&
                !personInfoElement.classList.contains("hidden");
              ...
              if (isRoomEligibleForShortLink(entry.personData)) {
                if (!isClosing) {
                  setShortLinkContext({
                    ...
                    codeOverride: normalizeShortCode(
                      entry.personData.ricerca,
                    ), // Use person ricerca for short code
                  });
                  updateURL(true);
                }
                // If closing, do not update URL to avoid map reset
              ...
              requestAnimationFrame(() => {
                if (hasDetails) {
                  ...
                  const expandIcon =
                    personElement.querySelector(".expand-icon");
                  if (expandIcon) {
                    expandIcon.style.transform =
                      personInfoElement.classList.contains("hidden") ?
                      "rotate(0deg)" :
      ...
      function expandRoomDetailsInSidebar(identifier) {
        ...
        const roomName =
          typeof identifier === "string" ? identifier : identifier.nome;
        if (!roomName) return;
        ...
        const matchesName = (element, name) => {
          ...
          return (
            textSpan &&
            (textSpan.textContent.trim() === name ||
        ...
        const allNavItems = roomsList.parentElement.querySelectorAll(
          ".nav-item, .department-item",
        ); // Includi anche i dipartimenti!
        ...
        for (const element of allNavItems) {
          if (
            matchesName(element, roomName) ||
            // Check Department data-name
            (element.classList.contains("department-item") &&
              element.getAttribute("data-name") === roomName)
          ) {
      ...
      function expandElementDetails(element) {
        const container =
          element.closest(".nav-item-container") ||
          element.closest(".department-item") ||
      ...
      function selectFloor(
        ...
        const shouldReload =
          selectedPolo !== lastLoadedPolo ||
          selectedBuilding !== lastLoadedBuilding ||
          floor !== lastLoadedFloor;
        selectedFloor = floor;

        if (selectedBuilding && floor !== "?") {
          checkForTopView(selectedBuilding, floor);
        } else {
          currentSelectionHasTopView = false;
          syncFlipViewControlState();
        }
        ...
        if (floor !== "?") updateURL(true); // Aggiorna l'URL ogni volta che si seleziona un piano
      }
      ...
      function syncFlipViewControlState() {
        if (!flipViewBtn) return;

        const shouldShowFlip =
          currentBuildingSupportsFlip && currentSelectionHasTopView;
        flipViewBtn.classList.toggle("hidden", !shouldShowFlip);
        flipViewBtn.disabled = !shouldShowFlip;
      }
      ...
      function updateFlipButtonVisibility(building) {
        ...
        if (
          building &&
          selectedPolo &&
          data.polo[selectedPolo] &&
          data.polo[selectedPolo].edificio[building]
        ) {
          shouldShow = !!data.polo[selectedPolo].edificio[building].rotate;
        }

        currentBuildingSupportsFlip = shouldShow;
      ...
      function checkForTopView(building, floor) {
        ...
        if (svgCache.has(cacheKey)) {
          ...
          currentSelectionHasTopView = exists;
          syncFlipViewControlState();
          return;
        }
        ...
        return fetch(svgPath)
          ...
          .then((svgData) => {
            ...
            const svgDataUrl =
              ...
              btoa(unescape(encodeURIComponent(modifiedSvgData)));
      ...
      function preloadAdjacentFloors(building, currentFloor) {
        ...
        const floors = Object.keys(edificio.piano)
          .map((f) => parseInt(f))
          .sort((a, b) => a - b);
      ...
      function showWaterDispensers() {
        ...
        if (
          ...
          waterDispenserOverlays.length > 0
        ) {
          return;
        }
          ...
          facilities.forEach((facility) => {
            if (facility.type === "erogatore_acqua" && facility.coordinates) {
              ...
              if (x != null && y != null) {
                ...
                const element = document.createElement("div");
                element.className =
                  "water-dispenser-marker counter-rotate-marker";
                ...
                element.setAttribute(
                  "data-room-name",
                  facility.nome || facility.ricerca || "",
                );
      ...
      function showStudyRoomMarkers() {
        ...
        try {
          ...
          console.error(
            "Errore nel mostrare i marker delle aule studio:",
            error,
          );
        }
      }
          ...
          .then((svgData) => {
            ...
            setTimeout(() => {
              ...
              if (viewBox) {
                ...
                width = parseFloat(svgElement.getAttribute("width")) || 1000;
                height =
                  parseFloat(svgElement.getAttribute("height")) || 1000;
              }

              // Serializza SVG modificato
              const serializer = new XMLSerializer();
              const modifiedSvgData =
      ...
      function renderCachedSVG(
        ...
        if (!viewer) {
          const isMobileDevice =
            typeof isRealMobileDevice === "function" ?
            isRealMobileDevice() :
            false;
          ...
          viewer.addHandler("canvas-drag", (e) => {
            if (isMapFlipped) {
              ...
              const viewportDelta = viewer.viewport.deltaPointsFromPixels(
                e.delta,
              );
          ...
          viewer.addHandler("canvas-scroll", (e) => {
            if (isMapFlipped) {
              ...
              const viewportPoint =
                viewer.viewport.pointFromPixel(invertedPoint);
          ...
          viewer.addHandler("canvas-pinch", (e) => {
            if (isMapFlipped) {
              ...
              const viewportDelta =
                viewer.viewport.deltaPointsFromPixels(delta);
              viewer.viewport.panBy(viewportDelta, true);
            }
          });
          ...
          viewer.addHandler("tile-loaded", (event) => {
            const altText =
              `Polo ${selectedPolo.charAt(0).toUpperCase() + selectedPolo.slice(1)}, Edificio ${selectedBuilding.toUpperCase()}, Piano ${selectedFloor}`;
            if (event.data) {
          ...
          if (
            mapContainer &&
            !mapContainer.hasAttribute("data-marker-delegation-installed")
          ) {
            mapContainer.setAttribute(
              "data-marker-delegation-installed",
              "true",
            );
        ...
        viewer.addOnceHandler("open", () => {
          ...
          if (
            ...
            roomData.coordinates.x != null &&
            roomData.coordinates.y != null
          ) {
            ...
            const zoom =
              roomData.coordinates.zoom != null ?
              parseFloat(roomData.coordinates.zoom) :
              2;
            // Use Image Zoom
            const viewportZoom = tiledImage.imageToViewportZoom(zoom);
      ...
      async function fetchCinecaEventsForCalendar(calendarId) {
        ...
        const fetchPromise = (async () => {
            ...
            try {
              ...
              console.error(
                "Cineca API fetch failed for calendar",
                calendarId,
                ":",
                e,
              );
      ...
      function getCalendarIdsForPolo(poloName) {
        ...
        if (poloData.edificio) {
          Object.entries(poloData.edificio).forEach(
            ([buildingKey, buildingData]) => {
      ...
      async function fetchAllEventsForPolo(poloName) {
        ...
        const allEvents = await Promise.all(
          calendarEntries.map((entry) =>
            fetchCinecaEventsForCalendar(entry.calendarId),
          ),
        );
      ...
      function getEventStatusString(event) {
        ...
        const docenti = (event.docenti || [])
          .map((d) => prettyProfName(d.nome, d.cognome))
          .join(", ");
      ...
      async function getAllSchedulesGivenPoleAndRoom(poleName, classroom) {
        ...
        events.forEach((event) => {
          const hasRoom = (event.aule || []).some(
            (a) => prettyAulaName(a.codice) === classroom,
          );
          if (hasRoom) {
      ...
      async function getFreeClassroomsNowGivenPole(poleName) {
        return {
          free_classrooms: []
        }; // Implementation complex, leaving empty
      }
       ...
       */
      async function getSchedulesPayloadGivenPole(
        poleName,
        classrooms = null,
      ) {
      ...
      function populateClassroomAvailability() {
        ...
        const polosToFetch = selectedPolo ?
          [selectedPolo] :
          Object.keys(data.polo);

        pendingApiRequests.set("batch_population", true);

        Promise.all(
          polosToFetch.map(async (poloName) => {
            // Capitalize for API: "fibonacci" -> "Fibonacci"
            const displayPoloName =
              poloName.charAt(0).toUpperCase() + poloName.slice(1);

            try {
              const payload =
                await getSchedulesPayloadGivenPole(displayPoloName);
              ...
              Object.values(poloData.edificio || {}).forEach((building) => {
                ...
                Object.values(building.piano || {}).forEach((floorData) => {
                  ...
                  rooms.forEach((room) => {
                    ...
                    if (
                      room.type &&
                      !["aula", "studio", "sala", "laboratorio"].includes(
                        room.type,
                      )
                    )
                      return;
                    ...
                    const apiKey =
                      room.cineca ||
                      room.nome.replace(/^Aula\s+/i, prefix + " ");
              ...
              console.warn(
                "Batch availability fetch failed for",
                poloName,
                err,
              );
            }
      ...
      async function getClassroomAvailability(classroom) {
        ...
        const poloName =
          colonIdx >= 0 ?
          classroom.substring(0, colonIdx) :
          selectedPolo || "fibonacci";
        const roomName =
          colonIdx >= 0 ? classroom.substring(colonIdx + 1) : classroom;
        ...
        if (poloData?.edificio) {
          for (const building of Object.values(poloData.edificio)) {
            if (building.prefix) {
              const buildingHasRoom = Object.values(
                building.piano || {},
              ).some((floorData) => {
        ...
        if (poloData?.edificio) {
          outer: for (const building of Object.values(poloData.edificio)) {
            for (const floorData of Object.values(building.piano || {})) {
              ...
              if (found?.cineca) {
                cinecaKey = found.cineca;
                break outer;
              }
            }
          }
        }
        const classroomApiName =
          cinecaKey || roomName.replace(/^Aula\s+/i, prefix + " ");
        const displayPoloName =
          poloName.charAt(0).toUpperCase() + poloName.slice(1);

        // Crea la promessa e salvala nella cache delle richieste pendenti
        const promise = getCurrentScheduleGivenPoleAndRoom(
            displayPoloName,
            classroomApiName,
          )
          .then((data) => {
      ...
      function hasVisibleOccupancyBars() {
        const desktopVisible =
          searchResults && !searchResults.classList.contains("hidden");
        const mobileVisible =
          searchResultsMobile &&
          !searchResultsMobile.classList.contains("hidden");
        ...
        if (
          desktopVisible &&
          searchResults.querySelector(".occupancy-status-bar")
        ) {
          return true;
        }
        ...
        if (
          mobileVisible &&
          searchResultsMobile.querySelector(".occupancy-status-bar")
        ) {
          return true;
        }
      ...
      function refreshSearchResultsOccupancy() {
        ...
        document
          .querySelectorAll(".occupancy-status-bar")
          .forEach((statusBar) => {
            ...
            const availabilityData =
              classroomName && window.classroomAvailability[classroomName];
            ...
            if (availabilityData && statusBar.classList.contains("loading")) {
              ...
              if (availabilityData.code === 0) {
                ...
                const textSpan = statusBar.querySelector(".occupancy-text");
                if (textSpan)
                  textSpan.textContent = t("availability_occupied");
                ...
                if (
                  ...
                  statusBar.addEventListener("click", (e) => {
                    e.stopPropagation();
                    const detailsElement =
                      document.getElementById(occupancyId);
                    if (detailsElement) {
                      detailsElement.classList.toggle("expanded");
                      expandIcon.classList.toggle("expanded");
                    }
